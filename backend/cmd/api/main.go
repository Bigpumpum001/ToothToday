...
func main() {
	...
	//line
	if err := clients.InitLineBot(); err != nil {
		log.Panic("Line Bot init failed:", err)
	}
	//db
	db.Connect()
	if err := clients.InitStorage(); err != nil {
	...
	r.POST("/api/services/:id/delete", middleware.AuthMiddleware(), handlers.SoftDeleteServiceContent)
	...
	// r.GET("/api/appointment/slot/day", middleware.AuthMiddleware(), handlers.GetAppointmentForAdmin)
	...
	if os.Getenv("GO_ENV") != "production" {
		...
		case "notify":
			...
			if err := services.NotifyUpcomingAppointments(ctx, db.Pool); err != nil {
		...
		case "complete":
			...
			if err := services.MarkCompleted(ctx, db.Pool); err != nil {
		...
		case "noshow":
			...
			if err := services.MarkNoShow(ctx, db.Pool); err != nil {
